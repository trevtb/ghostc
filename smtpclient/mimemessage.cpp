...
QString MimeMessage::toString()
{
    ...
    QLocale locale = QLocale( "en_GB");
    QDateTime cdt = QDateTime::currentDateTime();
    QString datestr = locale.toString(cdt, "ddd, dd MMM yyyy HH:mm:ss");

    QDateTime utcdate = cdt.toUTC();
    cdt.setTimeSpec(Qt::UTC);
    int offset = utcdate.secsTo(cdt) / 3600;

    datestr.append(" +");
    if (offset < 10) {
        datestr.append("0");
    } //endif
    datestr.append(QString::number(offset) + "00");

    switch (hEncoding)
    {
    case MimePart::Base64:
        mime += "=?utf-8?B?" + QByteArray().append(datestr).toBase64() + "?=";
        break;
    case MimePart::QuotedPrintable:
        mime += "=?utf-8?Q?" + QuotedPrintable::encode(QByteArray().append(datestr)).replace(' ', "_").replace(':',"=3A") + "?=";
        break;
    default:
        mime += datestr;
    }

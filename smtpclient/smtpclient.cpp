...
bool SmtpClient::connectToHost()
{
    ...
    case TlsConnection:
        // START EDIT
        connect((QSslSocket*) socket, SIGNAL(sslErrors(QList<QSslError>)),
...
// START EDIT
void SmtpClient::onSslErrors(QList<QSslError> errors) {
    std::cout<<"WARNING: SSL ERRORS OCCURED."<<std::endl;
    foreach (QSslError er, errors) {
        QString msg = er.errorString();
        std::cout << msg.toStdString().c_str() << std::endl;
    } //endforeach

    ((QSslSocket*)socket)->ignoreSslErrors();
} //endfunction onSslErrors
//END EDIT
